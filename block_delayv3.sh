...
# Credit Header
# Crafted by @chieb_sol vibecoding with @grok and @cursorAI - July 20th 2025
# Script to measure Bitcoin block propagation delays and send a Telegram summary - v3 with corrections to error handling, block delays and average calcs
# Designed for Bitcoin Knots (v28.1-4) on Umbrel (Docker)
...
# Dependencies: jq, curl, bc, awk, get_minerv3.sh
...
CURRENT_BLOCK=$(docker exec bitcoin-knots_app_1 bitcoin-cli -datadir=/data/bitcoin -rpcport=9332 getblockcount 2>>"$ERROR_LOG")
...
MINER_SCRIPT="$HOME/get_minerv3.sh"
...
TEMP_LOG=$(mktemp)
docker logs bitcoin-knots_app_1 > "$TEMP_LOG" 2>>"$ERROR_LOG"
...
if [ -n "$HEADER_LINE" ]; then
    HEADER_TIME=$(echo "$HEADER_LINE" | awk '{print $2}' | cut -d'.' -f1)
    BLOCK_HASH=$(echo "$HEADER_LINE" | grep -o "hash=[0-9a-f]\{64\}" | cut -d'=' -f2)
else
    BLOCK_HASH=""
    HEADER_TIME="N/A"
    echo "[$TIMESTAMP] Block $block: No 'Saw new header' found in logs or invalid format" >>"$ERROR_LOG"
    echo "Block $block: No 'Saw new header' found in logs or invalid format" >> "$LOG_DIR/Debug.log"
fi

VALIDATION_LINE=$(grep "UpdateTip: new best=.*height=$block" "$TEMP_LOG" | tail -1)
if [ -n "$VALIDATION_LINE" ]; then
    VALIDATION_TIME=$(echo "$VALIDATION_LINE" | awk '{print $2}' | cut -d'.' -f1)
    ...
    VALIDATION_TIME="N/A"
    MINED_TIME="N/A"
    echo "[$TIMESTAMP] Block $block: No 'UpdateTip' found in logs or invalid format" >>"$ERROR_LOG"
    echo "Block $block: No 'UpdateTip' found in logs or invalid format" >> "$LOG_DIR/Debug.log"
fi

echo "Block $block: HEADER_TIME=$HEADER_TIME, VALIDATION_TIME=$VALIDATION_TIME, MINED_TIME=$MINED_TIME" >> "$LOG_DIR/Debug.log"
    ...
    # ...rest of your block remains unchanged...

    EXTERNAL_MINED_TIME="N/A"
    EXTERNAL_EPOCH=""
    for attempt in {1..3}; do
        API_RESPONSE=$(curl -s "https://blockchain.info/block-height/$block?format=json")
        if echo "$API_RESPONSE" | jq empty 2>/dev/null; then
            EXTERNAL_EPOCH=$(echo "$API_RESPONSE" | jq -r '.blocks[0].time' 2>>"$ERROR_LOG")
            if [ -n "$EXTERNAL_EPOCH" ] && [[ "$EXTERNAL_EPOCH" =~ ^[0-9]+$ ]]; then
                break
            fi
        else
            echo "Block $block: Attempt $attempt failed, API did not return valid JSON" >> "$LOG_DIR/Debug.log"
        fi
    ...
    if [ -n "$EXTERNAL_EPOCH" ] && [[ "$EXTERNAL_EPOCH" =~ ^[0-9]+$ ]]; then
        ...
        echo "Block $block: Failed to fetch external mined time after 3 attempts. Last API response: $API_RESPONSE" >> "$LOG_DIR/Debug.log"
    fi
    sleep 2
    ...
    if [[ "$MINED_TIME" =~ $ISO8601_REGEX ]] && [[ "$HEADER_TIME" =~ $ISO8601_REGEX ]]; then
        ...
        MINED_TO_HEADER_DELAY="N/A"
        echo "[$TIMESTAMP] Block $block: Invalid date format for mined_time or header_time (mined_time=$MINED_TIME, header_time=$HEADER_TIME)" >>"$ERROR_LOG"
    fi

    if [[ "$MINED_TIME" =~ $ISO8601_REGEX ]] && [[ "$VALIDATION_TIME" =~ $ISO8601_REGEX ]]; then
        CLEAN_MINED_TIME="${MINED_TIME%%Z*}"
        ...
        MINED_TO_VALIDATION_DELAY="N/A"
        echo "[$TIMESTAMP] Block $block: Invalid date format for mined_time or validation_time (mined_time=$MINED_TIME, validation_time=$VALIDATION_TIME)" >>"$ERROR_LOG"
    fi
