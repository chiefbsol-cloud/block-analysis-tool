...
HEADER_DELAY_SUM=$(jq '[.blocks[].mined_to_header_delay_s | select(type == "number")] | add' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo "N/A")
VALIDATION_DELAY_SUM=$(jq '[.blocks[].mined_to_validation_delay_s | select(type == "number")] | add' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo "N/A")
