...
# Script to measure Bitcoin block propagation delays and send a Telegram summary
...
HEADER_DELAY_COUNT=$(jq '[.blocks[].mined_to_header_delay_s | select(type == "number")] | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo 0)
...
VALIDATION_DELAY_COUNT=$(jq '[.blocks[].mined_to_validation_delay_s | select(type == "number")] | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo 0)
...
if [ "$HEADER_DELAY_COUNT" -gt 0 ]; then
  AVG_HEADER_DELAY=$(jq '[.blocks[].mined_to_header_delay_s | select(type == "number")] | add / length | . * 10.0 | round / 10.0' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo "N/A")
else
  AVG_HEADER_DELAY="N/A"
fi

if [ "$VALIDATION_DELAY_COUNT" -gt 0 ]; then
  AVG_VALIDATION_DELAY=$(jq '[.blocks[].mined_to_validation_delay_s | select(type == "number")] | add / length | . * 10.0 | round / 10.0' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo "N/A")
else
...
MINER_1_NAME=$(echo "$MINER_DISTRIBUTION" | head -1 | awk '{print $2}' || echo "Unknown")
MINER_1_COUNT=$(echo "$MINER_DISTRIBUTION" | head -1 | awk '{print $1}' || echo "0")
MINER_2_NAME=$(echo "$MINER_DISTRIBUTION" | sed -n '2p' | awk '{print $2}' || echo "None")
MINER_2_COUNT=$(echo "$MINER_DISTRIBUTION" | sed -n '2p' | awk '{print $1}' || echo "0")
MINER_3_NAME=$(echo "$MINER_DISTRIBUTION" | sed -n '3p' | awk '{print $2}' || echo "None")
MINER_3_COUNT=$(echo "$MINER_DISTRIBUTION" | sed -n '3p' | awk '{print $1}' || echo "0")
MINER_OTHERS=$(jq -r '[.blocks[] | select(.mined_to_header_delay_s | type == "number" and . >= 0) | .miner_name] | unique | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" | awk '{print $1 - 3}' || echo 0)
if [ "$MINER_OTHERS" -lt 0 ]; then MINER_OTHERS=0; fi
...
DELAY_LT2=$(jq '[.blocks[] | select(.mined_to_header_delay_s | type == "number" and . >= 0 and . < -2)] | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo 0)
...
DELAY_LE6=$(jq '[.blocks[] | select(.mined_to_header_delay_s | type == "number" and . >= 0 and . > 1 and . <= 6)] | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo 0)
DELAY_LE10=$(jq '[.blocks[] | select(.mined_to_header_delay_s | type == "number" and . >= 0 and . > 6 and . <= 10)] | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo 0)
DELAY_LE15=$(jq '[.blocks[] | select(.mined_to_header_delay_s | type == "number" and . >= 0 and . > 10 and . <= 15)] | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo 0)
DELAY_LE20=$(jq '[.blocks[] | select(.mined_to_header_delay_s | type == "number" and . >= 0 and . > 15 and . <= 20)] | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo 0)
DELAY_GE21=$(jq '[.blocks[] | select(.mined_to_header_delay_s | type == "number" and . >= 0 and . >= 21)] | length' "$LOG_DIR/Propagation.json" 2>>"$ERROR_LOG" || echo 0)
...
echo "[$TIMESTAMP] Delay counts: LT2=$DELAY_LT2, LE1=$DELAY_LE1, LE6=$DELAY_LE6, LE10=$DELAY_LE10, LE15=$DELAY_LE15, LE20=$DELAY_LE20, GE21=$DELAY_GE21" >>"$ERROR_LOG"
echo "[$TIMESTAMP] Percentages: LT2=$PCT_LT2, LE1=$PCT_LE1, LE6=$PCT_LE6, LE10=$PCT_LE10, LE15=$PCT_LE15, LE20=$PCT_LE20, GE21=$PCT_GE21" >>"$ERROR_LOG"
...
🧩 Compact Blocks:  ${COMPACT_PERCENT}%
...
🛑 Negative Blocks: ${NEGATIVE_BLOCK:-0}
──────────────
