...
# Designed for Bitcoin Knots on Umbrel (Docker); outputs miner name for a given block height
# BITCOIN CORE USERS:
# - Umbrel: Replace 'bitcoin-knots_bitcoind_1' with your Bitcoin Core container name (e.g., 'bitcoin_bitcoind_1'). Run 'docker ps' to find it.
# - Native: Replace 'docker exec bitcoin-knots_bitcoind_1 bitcoin-cli' with 'bitcoin-cli' (e.g., 'bitcoin-cli getblockhash').
#   Ensure bitcoind is running ('systemctl status bitcoind') and ~/.bitcoin/bitcoin.conf has rpcuser/rpcpassword.
# Test your setup: Run 'bitcoin-cli getblockcount' to verify node access before executing.
# Dependencies: jq, xxd
...
get_block_hash() {
  ...
  # BITCOIN CORE USERS: Replace 'bitcoin-knots_bitcoind_1' with your container name or use 'bitcoin-cli getblockhash'.
  # Example: Umbrel Core: 'docker exec bitcoin_bitcoind_1 bitcoin-cli getblockhash "$block_height"'
  #          Native: 'bitcoin-cli getblockhash "$block_height"'
  local block_hash=$(docker exec bitcoin-knots_bitcoind_1 bitcoin-cli getblockhash "$block_height" 2>>"$ERROR_LOG")
...
get_miner_info() {
  local block_hash=$1
  # BITCOIN CORE USERS: Replace 'bitcoin-knots_bitcoind_1' with your container name or use 'bitcoin-cli getblock'.
  # Example: Umbrel Core: 'docker exec bitcoin_bitcoind_1 bitcoin-cli getblock "$block_hash" 2'
  #          Native: 'bitcoin-cli getblock "$block_hash" 2'
  local raw_block=$(docker exec bitcoin-knots_bitcoind_1 bitcoin-cli getblock "$block_hash" 2 2>>"$ERROR_LOG")
